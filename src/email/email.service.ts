...
export class EmailService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // ✅ PRIORIDADE 1: Usar Resend se disponível (melhor para cloud)
    if (resendApiKey) {
      this.resend = new Resend(resendApiKey);
      this.useResend = true;
      console.log('📧 Usando Resend para envio de emails (API HTTP)');
      console.log('   ✅ Melhor opção para ambientes cloud (Railway, Vercel, etc)');
      return;
    }
    
    // ✅ PRIORIDADE 2: Usar SMTP (local ou se Resend não configurado)
    if (!emailUser || !emailPass) {
      console.warn('⚠️ Nenhuma configuração de email encontrada.');
      console.warn('   Configure RESEND_API_KEY ou EMAIL_USER/EMAIL_PASS');
  ...
  async sendEmailVerification(email: string, token: string): Promise<void> {
    ...
    const fromEmail = this.configService.get('EMAIL_USER') || 'noreply@bravovip.com.br';

    const subject = '🎯 Verificação de E-mail - Bingo Live Bravo.Bet';
    
    // Se usar Resend
    if (this.useResend && this.resend) {
      await this.sendWithResend(email, subject, this.getVerificationEmailHtml(verificationUrl));
      return;
    }
  ...
  async sendPasswordReset(email: string, token: string): Promise<void> {
    ...
    const subject = '🔒 Redefinição de Senha - Bingo Live Bravo.Bet';
    
    // Se usar Resend
    if (this.useResend && this.resend) {
      await this.sendWithResend(email, subject, this.getPasswordResetEmailHtml(resetUrl));
      return;
    }
  ...
  /**
   * Enviar email usando Resend (API HTTP)
   */
